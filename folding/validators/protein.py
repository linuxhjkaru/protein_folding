...
class Protein(OpenMMSimulation):
    ...
    def gather_pdb_id(self):
        if self.pdb_id is None:
            ...
            print(f"Selected random pdb id: {self.pdb_id!r}")
    ...
    def generate_input_files(self):
        ...
        """
        print(f"Changing path to {self.pdb_directory}")
        ...
        print(f"Minimizing energy for pdb: {self.pdb_id} ...")

        start_time = time.time()
        # self.simulation.minimizeEnergy()  # TODO: figure out the right number for this
        self.simulation.minimizeEnergy(
            maxIterations=100
        )  # TODO: figure out the right number for this
        enrygyMin = self.simulation.context.getState(getEnergy=True).getPotentialEnergy() / unit.kilojoules_per_mole
        print(f"Minimization took {time.time() - start_time:.4f} seconds")
        print(f"enrygyMin: {enrygyMin}")
        ...
        print(f"Moving all files except pdb to {self.validator_directory}")
        os.system(cmd)
    ...
    def save_files(
        ...
        for filename, content in files.items():
            ...
            print(f"Saving file {filename} to {output_directory}")
    ...
    def process_md_output(
        ...
        try:
            ...
            print(f"Failed to recreate simulation: {e}")
            return False
    ...
    def check_gradient(self, check_energies: np.ndarray) -> True:
        ...
        WINODW size of the check_energies array. You guys that return gradients that are too high,
        ...
        print(f"check_energies[:WINDOW]: {check_energies[:WINDOW]}")
    ...
    def is_run_valid(self):
        ...
        print(f"median_percent_diff: {median_percent_diff}")
